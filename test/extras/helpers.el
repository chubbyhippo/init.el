...
(defvar extras-test--files
  '("clojure.el" "cobol.el" "cpp.el" "dotnet.el" "eglot-ensure.el" "elixir.el"
...
(defvar extras-test--menu-files
  (seq-remove (lambda (f) (equal f "eglot-ensure.el")) extras-test--files)
  "Every extras/*.el file with its own toggleable entry in extras.el's menu.
A strict subset of `extras-test--files' -- eglot-ensure.el is a shared
