;;; perl-tests.el --- ERT suite for extras/perl.el  -*- lexical-binding: t; -*-
...
;; Guide retained from extras/perl.el (moved here when that file's comments
...
;; perl-mode; a remap via `major-mode-remap-alist' is enough — no
;; auto-mode-alist fiddling. cperl-mode still derives from prog-mode, so
;; eglot keeps attaching.
;;
;; The indentation customizations match common Perl style (perlstyle.pod):
;; 4-space indent, hanging open paren blocks, closing paren pulled back. Tune
;; per-project with .dir-locals if a codebase disagrees. `cperl-invalid-face'
;; is set to nil because '_' in identifiers is not "invalid trailing
;; whitespace".
;;
...
;; provide `perl-mode' / `cperl-mode'); the extras file provides it anyway as
;; a courtesy, not a requirement. (Same reasoning as ruby.el / rust.el /
;; zig.el.)
...
;;; ================================================================= perl.el
(ert-deftest extras-test/given-perl-then-perl-mode-remaps-to-cperl-mode ()
  (should (extras-test--declares "perl.el" '(perl-mode . cperl-mode))))

(ert-deftest extras-test/given-perl-then-cperl-indentation-follows-perlstyle ()
  (should (member '(cperl-indent-level 4)
                  (extras-test--use-package-section "perl.el" 'cperl-mode :custom)))
  (should (member '(cperl-close-paren-offset -4)
                  (extras-test--use-package-section "perl.el" 'cperl-mode :custom)))
  (should (member '(cperl-invalid-face nil)
                  (extras-test--use-package-section "perl.el" 'cperl-mode :custom))))

(ert-deftest extras-test/given-perl-then-it-provides-perl ()
  (should (extras-test--declares "perl.el" '(provide 'perl))))

(provide 'perl-tests)
;;; perl-tests.el ends here
